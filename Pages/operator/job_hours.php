...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['booking_id'])) {
    ...
    $startTime = trim($_POST['operator_start_time'] ?? '');
    $endTime = trim($_POST['operator_end_time'] ?? '');

    $startDt = $startTime !== '' ? date('Y-m-d H:i:s', strtotime($startTime)) : null;
    $endDt = $endTime !== '' ? date('Y-m-d H:i:s', strtotime($endTime)) : null;
    ...
    if ($startDt && $endDt && strtotime($endDt) < strtotime($startDt)) {
        ...
        try {
            ...
            $sql = "UPDATE bookings SET operator_start_time = ?, operator_end_time = ?, updated_at = NOW() WHERE booking_id = ? AND operator_id = ?";
            if ($stmt = $conn->prepare($sql)) {
                $stmt->bind_param('ssii', $startDt, $endDt, $bookingId, $operatorId);
                if ($stmt->execute()) {
            ...
            error_log('Operator job hours update error: ' . $e->getMessage());
...
if ($bookingId > 0) {
    try {
        $conn = getDBConnection();
        $sql = "SELECT b.booking_id, b.operator_start_time, b.operator_end_time, b.booking_date,
                       e.equipment_name,
        ...
        error_log('Operator job hours fetch error: ' . $e->getMessage());
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Record Work Hours - FES Operator</title>
            ...
            <?php else: ?>
                <section class="bg-white rounded-xl shadow-card p-6 max-w-2xl">
                    <div class="mb-4 text-sm text-gray-600">
                        <?php echo htmlspecialchars($booking['customer_name'] ?? 'N/A'); ?> · <?php echo htmlspecialchars($booking['equipment_name'] ?? ''); ?> · <?php echo !empty($booking['booking_date']) ? htmlspecialchars(date('M d, Y', strtotime($booking['booking_date']))) : 'N/A'; ?>
                    </div>
                    <form method="post" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="operator_start_time" value="<?php echo !empty($booking['operator_start_time']) ? htmlspecialchars(date('Y-m-d\TH:i', strtotime($booking['operator_start_time']))) : ''; ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fes-red focus:border-fes-red">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">End Time</label>
                            <input type="datetime-local" name="operator_end_time" value="<?php echo !empty($booking['operator_end_time']) ? htmlspecialchars(date('Y-m-d\TH:i', strtotime($booking['operator_end_time']))) : ''; ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fes-red focus:border-fes-red">
                        </div>
